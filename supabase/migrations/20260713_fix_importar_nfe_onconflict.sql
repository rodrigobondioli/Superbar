-- ============================================================
-- FIX — ON CONFLICT do upsert de fornecedor não casava com o índice.
--
-- A unicidade de fornecedores é um ÍNDICE PARCIAL:
--   create unique index ... on fornecedores(bar_id, cnpj) where cnpj is not null;
-- O Postgres só usa índice parcial como árbitro de ON CONFLICT se a cláusula
-- REPETIR o predicado. O RPC fazia `on conflict (bar_id, cnpj)` sem o
-- `where cnpj is not null` → erro:
--   "there is no unique or exclusion constraint matching the ON CONFLICT specification"
-- Isso abortava a transação inteira e NADA era importado.
--
-- Correção: adicionar o predicado no ON CONFLICT do fornecedor. O insert só roda
-- quando p_cnpj não é nulo (guard acima), então o predicado casa sempre.
-- Nada mais muda em relação à versão anterior (20260711_importar_nfe_tamanho).
-- ============================================================
...
begin
  ...
  -- Fornecedor (upsert por CNPJ). O ON CONFLICT repete o predicado do índice parcial.
  if p_cnpj is not null and length(trim(p_cnpj)) > 0 then
    ...
    on conflict (bar_id, cnpj) where cnpj is not null do update set nome = excluded.nome
